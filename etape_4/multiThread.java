...
public void run() /*throws InterruptedException*/{
	System.out.println( "le client connect� ("+this.getName()+"): "+client.getInetAddress().toString());
	...
	try {
		...
		cote_serv.L1.add(O);
		System.out.println("La liste du "+this.getName());
		for( Field f : O.getClass().getDeclaredFields() ) {
			f.setAccessible(true);
			String finc = f.getType().getName();
			System.out.println(f.get(O)+" est un "+finc.substring(finc.lastIndexOf('.')+1, finc.length()));}
